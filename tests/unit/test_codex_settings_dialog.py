"""GUI wiring for the Codex SDK provider backend."""

from __future__ import annotations

from unittest.mock import patch

from pa_agent.config.settings import Settings
from pa_agent.gui.ai_model_settings_dialog import AIModelSettingsDialog
from pa_agent.gui.settings_dialog import SettingsDialog


def _select_codex(dialog) -> None:
    index = dialog._backend_combo.findData("codex_sdk")
    assert index >= 0
    dialog._backend_combo.setCurrentIndex(index)


def _assert_codex_fields(dialog) -> None:
    assert dialog._model_edit.text() == "gpt-5.6-terra"
    assert dialog._backend_combo.count() == 1
    assert dialog._backend_combo.currentData() == "codex_sdk"
    assert not dialog._backend_combo.isEnabled()
    assert not dialog._base_url_edit.isEnabled()
    assert not dialog._api_key_edit.isEnabled()
    assert "无需 API Key" in dialog._api_key_edit.placeholderText()
    assert dialog._reasoning_effort_combo.currentText() == "high"
    assert not dialog._reasoning_effort_combo.isEnabled()


def test_ai_model_dialog_exposes_codex_backend(qtbot) -> None:
    dialog = AIModelSettingsDialog(Settings())
    qtbot.addWidget(dialog)
    _select_codex(dialog)
    _assert_codex_fields(dialog)


def test_full_settings_dialog_exposes_codex_backend(qtbot) -> None:
    dialog = SettingsDialog(Settings())
    qtbot.addWidget(dialog)
    _select_codex(dialog)
    _assert_codex_fields(dialog)


def test_ai_model_dialog_save_sanitizes_legacy_provider(qtbot) -> None:
    settings = Settings()
    settings.provider.backend = "openai_compatible"
    settings.provider.model = "deepseek-v4-flash"
    settings.provider.base_url = "https://api.deepseek.com"
    settings.provider.api_key = "legacy-secret"
    dialog = AIModelSettingsDialog(settings)
    qtbot.addWidget(dialog)

    with patch("pa_agent.gui.ai_model_settings_dialog.save_settings") as save:
        dialog._on_save()

    save.assert_called_once()
    assert settings.provider.backend == "codex_sdk"
    assert settings.provider.model == "gpt-5.6-terra"
    assert settings.provider.base_url == ""
    assert settings.provider.api_key == ""
    assert settings.provider.reasoning_effort == "high"


def test_full_settings_dialog_save_sanitizes_legacy_provider(qtbot) -> None:
    settings = Settings()
    settings.provider.backend = "cursor_sdk"
    settings.provider.model = "openclaw_cs"
    settings.provider.api_key = "legacy-secret"
    dialog = SettingsDialog(settings)
    qtbot.addWidget(dialog)

    with patch("pa_agent.gui.settings_dialog.save_settings") as save:
        dialog._on_save()

    save.assert_called_once()
    assert settings.provider.backend == "codex_sdk"
    assert settings.provider.model == "gpt-5.6-terra"
    assert settings.provider.base_url == ""
    assert settings.provider.api_key == ""
    assert settings.provider.reasoning_effort == "high"
